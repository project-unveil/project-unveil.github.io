"""One-off: regenerate exercising_predicted.bin / exercising_gt.bin
with gender='male' so the SMPL body shape matches the metadata flip.
Run with:
    C:/Users/sihat/miniconda3/envs/animation/python.exe _regen_exercising.py
"""
...
GT   = dict(height=167, weight=52, gender="male")
PRED = dict(height=163, weight=57, gender="male")
...
def main():
    ...
    save_verts(vp, fps, os.path.join(SMPL_OUT, "exercising_predicted.bin"))

    print("Fitting GT (male)...")
    vg, fps2 = run_smpl(BVH, GT["height"], GT["weight"], GT["gender"])
    save_verts(vg, fps2, os.path.join(SMPL_OUT, "exercising_gt.bin"))
