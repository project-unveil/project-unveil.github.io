"""One-off: regen dancing_predicted.bin + dancing_gt.bin with male SMPL,
preserving the original 300-frame downsampling (rest of the demos use 150).
"""
...
PRED = dict(height=168, weight=68, gender="male")
GT   = dict(height=172, weight=68, gender="male")
...
def main():
    ...
    save_verts(vp, fps, os.path.join(SMPL_OUT, "dancing_predicted.bin"))

    print("Fitting GT (male)...")
    vg, fps2 = run_smpl(BVH, GT["height"], GT["weight"], GT["gender"])
    save_verts(vg, fps2, os.path.join(SMPL_OUT, "dancing_gt.bin"))
