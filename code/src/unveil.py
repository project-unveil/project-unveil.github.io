...
_SCRIPT_DIR = Path(__file__).resolve().parent   # Submission/
_BONES_ROOT = _SCRIPT_DIR.parent               # bones-seed/ (project_paths.py lives here)
_DYNGRAPH_ROOT = _SCRIPT_DIR / "DS-GCN"       # bundled inside Submission/
_PROTOMEM_ROOT = _SCRIPT_DIR / "ProtoGCN"     # bundled inside Submission/

# project_paths.py lives one level up in the bones-seed root
...
# Pre-register the local pyskl stub (bundled in Submission/) before DS-GCN is added
...
def run_single_task(args: argparse.Namespace) -> Dict:
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for xb, lengths, yb, tids in train_loader:
            ...
            if is_regression:
                loss = ce_loss_fn(logits.squeeze(1), yb)
            else:
                ...
                if (mem_contrast_fn is not None and args.lambda_proto > 0
        ...
        tr_ce = ce_sum / max(1, total) if not is_regression else tr_loss
